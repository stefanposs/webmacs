...
class SQLAlchemyDatapointRepository:
    ...
    async def get_latest_per_event(self) -> Sequence[DatapointRecord]:
        ...
        result = await self._db.execute(
            select(Datapoint).join(
                subq,
                (Datapoint.event_public_id == subq.c.event_public_id)
                & (Datapoint.timestamp == subq.c.max_ts),
            )
        )
    ...
    async def list_paginated(
        ...
        result = await self._db.execute(
            select(Datapoint)
            .order_by(desc(Datapoint.timestamp))
    ...
    async def get_by_public_id(self, public_id: str) -> DatapointRecord | None:
        result = await self._db.execute(
            select(Datapoint).where(Datapoint.public_id == public_id)
        )
